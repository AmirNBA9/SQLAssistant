...
SELECT
	...
	DF.DataFilesCount AS DataFilesCount,
	DF.DataSize_GB AS DataSize_GB,
	DF.LogFilesCount AS LogFilesCount,
	DF.LogSize_GB AS LogSize_GB,
	DB.user_access_desc AS UserAccessType,
	...
	DB.collation_name AS CollationNameDataBase,
	DB.catalog_collation_type_desc AS CatalogCollationType,
	DB.snapshot_isolation_state_desc AS SnapshotIsolationStateDescription,
	DB.is_ansi_nulls_on AS IsAnsiNull,
	DB.is_ansi_warnings_on AS IsAnsiWarning,
	DB.is_fulltext_enabled AS FullTextState,
	DB.is_trustworthy_on AS TrustWorthyState,
	DB.is_master_key_encrypted_by_server AS MasterKeyServer,
	DB.is_distributor AS Distreibutor,
	...
	DB.is_published AS IsPublishedInReplication,
	DB.is_temporal_history_retention_enabled AS TemporalHistpryRetention,
	DB.is_memory_optimized_enabled AS MemoryOptimized,
	...
	is_accelerated_database_recovery_on	AS IsAcceleratedDatabaseRecoveryOn-- Starting with SQL Server 2019 (15.x) and Azure SQL Database
FROM
	sys.databases AS DB
CROSS APPLY
(
	SELECT
		COUNT (CASE WHEN [type] = 0 THEN [type] END) AS DataFilesCount,
