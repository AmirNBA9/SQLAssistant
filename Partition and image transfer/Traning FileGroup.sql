...
CREATE PARTITION FUNCTION pfOrderdateRange (DATETIME) AS RANGE LEFT FOR VALUES ('2016/12/31', '2017/12/31', '2018/12/31');
GO

CREATE PARTITION SCHEME psOderdateRange AS PARTITION pfOrderDateRange TO (FG1, FG2, FG3, [PRIMARY]);
GO
...
CREATE TABLE Orders (Orderid      INT      IDENTITY(1, 1) NOT NULL,
                     ...
                     ProductID    INT      NULL,
                     CONSTRAINT PK_Orders PRIMARY KEY CLUSTERED (Orderid ASC, Orderdate ASC) ON psOderdateRange(orderdate)) ON psOderdateRange (Orderdate);
GO
...
SELECT OBJECT_NAME (i.object_id) AS OBJECT_NAME, p.partition_number, fg.name AS FILEGROUP_NAME, p.rows, au.total_pages, CASE f.boundary_value_on_right
                                                                                                                            WHEN 1 THEN 'Less than'
                                                                                                                            ELSE 'Less or equal than'
                                                                                                                        END AS 'Comparition', rv.value
...
ALTER PARTITION SCHEME psOderdateRange
NEXT USED FG4;
GO
...
ADD FILE (NAME = P4, FILENAME = N'C:\Program Files\Microsoft SQL Server\MSSQL14.MSSQLSERVER2017\MSSQL\DATA\P4.NDF', SIZE = 1024KB, MAXSIZE = UNLIMITED, FILEGROWTH = 10%)
TO FILEGROUP FG4;
--
GO
...
CREATE TABLE dbo.Orders_Temp (OrderID      INT      IDENTITY(1, 1) NOT NULL,
                              OrderDate    DATETIME NOT NULL,
                              OrderFreight MONEY    NULL,
                              ProductID    INT      NULL,
                              CONSTRAINT PK_OrdersTemp PRIMARY KEY CLUSTERED (OrderID ASC, OrderDate ASC) ON FG1) ON FG1;
GO

USE tempdb;
GO

CREATE TABLE dbo.Orders_Hist (OrderID      INT      NOT NULL,
                              OrderDate    DATETIME NOT NULL,
                              OrderFreight MONEY    NULL,
                              ProductID    INT      NULL,
