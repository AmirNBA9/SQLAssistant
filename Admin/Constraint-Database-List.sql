...
*/
--------------------------------------------------------------------
SELECT	   table_view, object_type, constraint_type, constraint_name, details
  FROM	   (SELECT SCHEMA_NAME (t.schema_id) + '.' + t.[Name] AS table_view, --
				   ...
				   END AS constraint_type, ISNULL (c.[Name], i.[Name]) AS constraint_name, SUBSTRING (column_names, 1, LEN (column_names) - 1) AS [details]
			  FROM sys.objects t
			...
			UNION ALL
			SELECT SCHEMA_NAME (fk_tab.schema_id) + '.' + fk_tab.Name AS foreign_table, 'Table', 'Foreign key', fk.Name AS fk_constraint_name, SCHEMA_NAME (pk_tab.schema_id) + '.' + pk_tab.Name
			...
			UNION ALL
			SELECT SCHEMA_NAME (t.schema_id) + '.' + t.[Name], 'Table', 'Check constraint', con.[Name] AS constraint_name, con.[definition]
			  FROM sys.check_constraints con
				   LEFT OUTER JOIN sys.objects t ON con.parent_object_id = t.object_id
				   LEFT OUTER JOIN sys.all_columns col ON con.parent_column_id = col.column_id
													  AND con.parent_object_id = col.object_id
			UNION ALL
			SELECT SCHEMA_NAME (t.schema_id) + '.' + t.[Name], 'Table', 'Default constraint', con.[Name], col.[Name] + ' = ' + con.[definition]
