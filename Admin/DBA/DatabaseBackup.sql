IF OBJECT_ID('[dba].[DatabaseBackup]') IS NOT NULL
	DROP PROCEDURE [dba].[DatabaseBackup];
...
CREATE       PROCEDURE [dba].[DatabaseBackup]
...
DECLARE @LogToTable NVARCHAR(MAX) =  N'N'
DECLARE @FileExtensionFull NVARCHAR(MAX) = NULL
DECLARE @FileExtensionDiff NVARCHAR(MAX) = NULL
DECLARE @FileExtensionLog NVARCHAR(MAX) = NULL;
DECLARE @DirectoryStructure nvarchar(max) = '{ServerName}${InstanceName}{DirectorySeparator}{DatabaseName}{DirectorySeparator}{BackupType}_{Partial}_{CopyOnly}'
DECLARE @AvailabilityGroupDirectoryStructure nvarchar(max) = '{ClusterName}${AvailabilityGroupName}{DirectorySeparator}{DatabaseName}{DirectorySeparator}{BackupType}_{Partial}_{CopyOnly}'
DECLARE @FileName nvarchar(max) = '{ServerName}${InstanceName}_{DatabaseName}_{BackupType}_{Partial}_{CopyOnly}_{Year}{Month}{Day}_{Hour}{Minute}{Second}_{FileNumber}.{FileExtension}'
DECLARE @AvailabilityGroupFileName nvarchar(max) = '{ClusterName}${AvailabilityGroupName}_{DatabaseName}_{BackupType}_{Partial}_{CopyOnly}_{Year}{Month}{Day}_{Hour}{Minute}{Second}_{FileNumber}.{FileExtension}',
...
@i int
  ...
  SET @Parameters = N'@Databases = ' + ISNULL('''' + REPLACE(@Databases,'''','''''') + '''','NULL');
  SET @Parameters += N', @Directory = ' + ISNULL('''' + REPLACE(@Directory,'''','''''') + '''','NULL');
  SET @Parameters += N', @BackupType = ' + ISNULL('''' + REPLACE(@BackupType,'''','''''') + '''','NULL');
  SET @Parameters += N', @Verify = ' + ISNULL('''' + REPLACE(@Verify,'''','''''') + '''','NULL');
  SET @Parameters += N', @CleanupTime = ' + ISNULL(CAST(@CleanupTime AS nvarchar),'NULL');
  SET @Parameters += N', @CleanupMode = ' + ISNULL('''' + REPLACE(@CleanupMode,'''','''''') + '''','NULL');
  SET @Parameters += N', @Compress = ' + ISNULL('''' + REPLACE(@Compress,'''','''''') + '''','NULL');
  SET @Parameters += N', @CopyOnly = ' + ISNULL('''' + REPLACE(@CopyOnly,'''','''''') + '''','NULL');
  SET @Parameters += N', @ChangeBackupType = ' + ISNULL('''' + REPLACE(@ChangeBackupType,'''','''''') + '''','NULL');
  SET @Parameters += N', @BackupSoftware = ' + ISNULL('''' + REPLACE(@BackupSoftware,'''','''''') + '''','NULL');
  SET @Parameters += N', @CheckSum = ' + ISNULL('''' + REPLACE(@CheckSum,'''','''''') + '''','NULL');
  SET @Parameters += N', @BlockSize = ' + ISNULL(CAST(@BlockSize AS nvarchar),'NULL');
  SET @Parameters += N', @BufferCount = ' + ISNULL(CAST(@BufferCount AS nvarchar),'NULL');
  SET @Parameters += N', @MaxTransferSize = ' + ISNULL(CAST(@MaxTransferSize AS nvarchar),'NULL');
  SET @Parameters += N', @NumberOfFiles = ' + ISNULL(CAST(@NumberOfFiles AS nvarchar),'NULL');
  SET @Parameters += N', @MinBackupSizeForMultipleFiles = ' + ISNULL(CAST(@MinBackupSizeForMultipleFiles AS nvarchar),'NULL');
  SET @Parameters += N', @MaxFileSize = ' + ISNULL(CAST(@MaxFileSize AS nvarchar),'NULL');
  SET @Parameters += N', @CompressionLevel = ' + ISNULL(CAST(@CompressionLevel AS nvarchar),'NULL');
  SET @Parameters += N', @Description = ' + ISNULL('''' + REPLACE(@Description,'''','''''') + '''','NULL');
  SET @Parameters += N', @Threads = ' + ISNULL(CAST(@Threads AS nvarchar),'NULL');
  SET @Parameters += N', @Throttle = ' + ISNULL(CAST(@Throttle AS nvarchar),'NULL');
  SET @Parameters += N', @Encrypt = ' + ISNULL('''' + REPLACE(@Encrypt,'''','''''') + '''','NULL');
  SET @Parameters += N', @EncryptionAlgorithm = ' + ISNULL('''' + REPLACE(@EncryptionAlgorithm,'''','''''') + '''','NULL');
  SET @Parameters += N', @ServerCertificate = ' + ISNULL('''' + REPLACE(@ServerCertificate,'''','''''') + '''','NULL');
  SET @Parameters += N', @ServerAsymmetricKey = ' + ISNULL('''' + REPLACE(@ServerAsymmetricKey,'''','''''') + '''','NULL');
  SET @Parameters += N', @EncryptionKey = ' + ISNULL('''' + REPLACE(@EncryptionKey,'''','''''') + '''','NULL');
  SET @Parameters += N', @ReadWriteFileGroups = ' + ISNULL('''' + REPLACE(@ReadWriteFileGroups,'''','''''') + '''','NULL');
  SET @Parameters += N', @OverrideBackupPreference = ' + ISNULL('''' + REPLACE(@OverrideBackupPreference,'''','''''') + '''','NULL');
  SET @Parameters += N', @NoRecovery = ' + ISNULL('''' + REPLACE(@NoRecovery,'''','''''') + '''','NULL');
  SET @Parameters += N', @URL = ' + ISNULL('''' + REPLACE(@URL,'''','''''') + '''','NULL');
  SET @Parameters += N', @Credential = ' + ISNULL('''' + REPLACE(@Credential,'''','''''') + '''','NULL');
  SET @Parameters += N', @MirrorDirectory = ' + ISNULL('''' + REPLACE(@MirrorDirectory,'''','''''') + '''','NULL');
  SET @Parameters += N', @MirrorCleanupTime = ' + ISNULL(CAST(@MirrorCleanupTime AS nvarchar),'NULL');
  SET @Parameters += N', @MirrorCleanupMode = ' + ISNULL('''' + REPLACE(@MirrorCleanupMode,'''','''''') + '''','NULL');
  SET @Parameters += N', @MirrorURL = ' + ISNULL('''' + REPLACE(@MirrorURL,'''','''''') + '''','NULL');
  SET @Parameters += N', @AvailabilityGroups = ' + ISNULL('''' + REPLACE(@AvailabilityGroups,'''','''''') + '''','NULL');
  SET @Parameters += N', @Updateability = ' + ISNULL('''' + REPLACE(@Updateability,'''','''''') + '''','NULL');
  SET @Parameters += N', @AdaptiveCompression = ' + ISNULL('''' + REPLACE(@AdaptiveCompression,'''','''''') + '''','NULL');
  SET @Parameters += N', @ModificationLevel = ' + ISNULL(CAST(@ModificationLevel AS nvarchar),'NULL');
  SET @Parameters += N', @LogSizeSinceLastLogBackup = ' + ISNULL(CAST(@LogSizeSinceLastLogBackup AS nvarchar),'NULL');
  SET @Parameters += N', @TimeSinceLastLogBackup = ' + ISNULL(CAST(@TimeSinceLastLogBackup AS nvarchar),'NULL');
  SET @Parameters += N', @DataDomainBoostHost = ' + ISNULL('''' + REPLACE(@DataDomainBoostHost,'''','''''') + '''','NULL');
  SET @Parameters += N', @DataDomainBoostUser = ' + ISNULL('''' + REPLACE(@DataDomainBoostUser,'''','''''') + '''','NULL');
  SET @Parameters += N', @DataDomainBoostDevicePath = ' + ISNULL('''' + REPLACE(@DataDomainBoostDevicePath,'''','''''') + '''','NULL');
  SET @Parameters += N', @DataDomainBoostLockboxPath = ' + ISNULL('''' + REPLACE(@DataDomainBoostLockboxPath,'''','''''') + '''','NULL');
  SET @Parameters += N', @DirectoryStructure = ' + ISNULL('''' + REPLACE(@DirectoryStructure,'''','''''') + '''','NULL');
  SET @Parameters += N', @AvailabilityGroupDirectoryStructure = ' + ISNULL('''' + REPLACE(@AvailabilityGroupDirectoryStructure,'''','''''') + '''','NULL');
  SET @Parameters += N', @FileName = ' + ISNULL('''' + REPLACE(@FileName,'''','''''') + '''','NULL');
  SET @Parameters += N', @AvailabilityGroupFileName = ' + ISNULL('''' + REPLACE(@AvailabilityGroupFileName,'''','''''') + '''','NULL');
  SET @Parameters += N', @FileExtensionFull = ' + ISNULL('''' + REPLACE(@FileExtensionFull,'''','''''') + '''','NULL');
  SET @Parameters += N', @FileExtensionDiff = ' + ISNULL('''' + REPLACE(@FileExtensionDiff,'''','''''') + '''','NULL');
  SET @Parameters += N', @FileExtensionLog = ' + ISNULL('''' + REPLACE(@FileExtensionLog,'''','''''') + '''','NULL');
  SET @Parameters += N', @Init = ' + ISNULL('''' + REPLACE(@Init,'''','''''') + '''','NULL');
  SET @Parameters += N', @Format = ' + ISNULL('''' + REPLACE(@Format,'''','''''') + '''','NULL');
  SET @Parameters += N', @ObjectLevelRecoveryMap = ' + ISNULL('''' + REPLACE(@ObjectLevelRecoveryMap,'''','''''') + '''','NULL');
  SET @Parameters += N', @StringDelimiter = ' + ISNULL('''' + REPLACE(@StringDelimiter,'''','''''') + '''','NULL');
  SET @Parameters += N', @DatabaseOrder = ' + ISNULL('''' + REPLACE(@DatabaseOrder,'''','''''') + '''','NULL');
  SET @Parameters += N', @DatabasesInParallel = ' + ISNULL('''' + REPLACE(@DatabasesInParallel,'''','''''') + '''','NULL');
  SET @Parameters += N', @LogToTable = ' + ISNULL('''' + REPLACE(@LogToTable,'''','''''') + '''','NULL');
  SET @Parameters += N', @Execute = ' + ISNULL('''' + REPLACE(@Execute,'''','''''') + '''','NULL');

  SET @StartMessage = N'Date and time: ' + CONVERT(nvarchar,@StartTime,120);
  RAISERROR('%s',10,1,@StartMessage) WITH NOWAIT;

  SET @StartMessage = N'Server: ' + CAST(SERVERPROPERTY('ServerName') AS nvarchar(max));
  RAISERROR('%s',10,1,@StartMessage) WITH NOWAIT;

  SET @StartMessage = N'Version: ' + CAST(SERVERPROPERTY('ProductVersion') AS nvarchar(max));
  RAISERROR('%s',10,1,@StartMessage) WITH NOWAIT;

  SET @StartMessage = N'Edition: ' + CAST(SERVERPROPERTY('Edition') AS nvarchar(max));
  RAISERROR('%s',10,1,@StartMessage) WITH NOWAIT;

  SET @StartMessage = N'Platform: ' + @HostPlatform;
  RAISERROR('%s',10,1,@StartMessage) WITH NOWAIT;

  SET @StartMessage = N'Procedure: ' + QUOTENAME(DB_NAME(DB_ID())) + N'.' + QUOTENAME(@SchemaName) + N'.' + QUOTENAME(@ObjectName);
  RAISERROR('%s',10,1,@StartMessage) WITH NOWAIT;

  SET @StartMessage = N'Parameters: ' + @Parameters;
  RAISERROR('%s',10,1,@StartMessage) WITH NOWAIT;

  SET @StartMessage = N'Version: ' + @VersionTimestamp;
  RAISERROR('%s',10,1,@StartMessage) WITH NOWAIT;

  SET @StartMessage = N'Source: https://ola.hallengren.com';
  RAISERROR('%s',10,1,@StartMessage) WITH NOWAIT;
  ...
  SET @ErrorMessage = N'';
  SELECT @ErrorMessage = @ErrorMessage + QUOTENAME(DatabaseName) + N', '
  FROM @tmpDatabases
  WHERE Selected = 1
  AND DatabaseNameFS = ''
  ...
  SET @ErrorMessage = N'';
  SELECT @ErrorMessage = @ErrorMessage + QUOTENAME(AvailabilityGroupName) + N', '
...
;  ELSE
 ...
 ; ELSE
  ...
  BEGIN
    ...
    SET @CurrentDatabase_sp_executesql = QUOTENAME(@CurrentDatabaseName) + N'.sys.sp_executesql';

    BEGIN
      SET @DatabaseMessage = N'Date and time: ' + CONVERT(nvarchar,SYSDATETIME(),120);
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;

      SET @DatabaseMessage = N'Database: ' + QUOTENAME(@CurrentDatabaseName);
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;
    END;
    ...
    BEGIN
      SET @DatabaseMessage = N'State: ' + @CurrentDatabaseState
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;

      SET @DatabaseMessage = N'Standby: ' + CASE WHEN @CurrentInStandby = 1 THEN 'Yes' ELSE 'No' END;
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;

      SET @DatabaseMessage =  N'Updateability: ' + CASE WHEN @CurrentIsReadOnly = 1 THEN 'READ_ONLY' WHEN  @CurrentIsReadOnly = 0 THEN 'READ_WRITE' END;
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;

      SET @DatabaseMessage =  N'User access: ' + @CurrentUserAccess;
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;

      SET @DatabaseMessage = N'Recovery model: ' + @CurrentRecoveryModel;
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;

      SET @DatabaseMessage = N'Encrypted: ' + CASE WHEN @CurrentIsEncrypted = 1 THEN 'Yes' WHEN @CurrentIsEncrypted = 0 THEN 'No' ELSE 'N/A' END;
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;
    END;
    ...
    BEGIN
      SET @CurrentCommand = N'SELECT @ParamAllocatedExtentPageCount = SUM(allocated_extent_page_count), @ParamModifiedExtentPageCount = SUM(modified_extent_page_count) FROM sys.dm_db_file_space_usage';
    ...
    BEGIN
      SET @CurrentLogShippingRole = N'PRIMARY';
    END;
    ELSE
    IF EXISTS (SELECT * FROM msdb.dbo.log_shipping_secondary_databases WHERE secondary_database = @CurrentDatabaseName)
    BEGIN
      SET @CurrentLogShippingRole = N'SECONDARY';
    END;

    IF @CurrentIsDatabaseAccessible IS NOT NULL
    BEGIN
      SET @DatabaseMessage = N'Is accessible: ' + CASE WHEN @CurrentIsDatabaseAccessible = 1 THEN 'Yes' ELSE 'No' END;
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;
    END;

    IF @CurrentAvailabilityGroup IS NOT NULL
    BEGIN
      SET @DatabaseMessage = N'Availability group: ' + ISNULL(@CurrentAvailabilityGroup,'N/A');
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;

      SET @DatabaseMessage = N'Availability group role: ' + ISNULL(@CurrentAvailabilityGroupRole,'N/A');
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;

      SET @DatabaseMessage = N'Availability group backup preference: ' + ISNULL(@CurrentAvailabilityGroupBackupPreference,'N/A');
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;

      SET @DatabaseMessage = N'Is preferred backup replica: ' + CASE WHEN @CurrentIsPreferredBackupReplica = 1 THEN 'Yes' WHEN @CurrentIsPreferredBackupReplica = 0 THEN 'No' ELSE 'N/A' END;
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;
    END;

    IF @CurrentDatabaseMirroringRole IS NOT NULL
    BEGIN
      SET @DatabaseMessage = N'Database mirroring role: ' + @CurrentDatabaseMirroringRolE
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;
    END;

    IF @CurrentLogShippingRole IS NOT NULL
    BEGIN
      SET @DatabaseMessage = N'Log shipping role: ' + @CurrentLogShippingRolE
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;
    END;

    SET @DatabaseMessage = N'Differential base LSN: ' + ISNULL(CAST(@CurrentDifferentialBaseLSN AS nvarchar),'N/A');
    RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;

    IF @CurrentBackupType = 'DIFF'
    BEGIN
      SET @DatabaseMessage = N'Differential base is snapshot: ' + CASE WHEN @CurrentDifferentialBaseIsSnapshot = 1 THEN 'Yes' WHEN @CurrentDifferentialBaseIsSnapshot = 0 THEN 'No' ELSE 'N/A' END;
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;
    END;

    SET @DatabaseMessage = N'Last log backup LSN: ' + ISNULL(CAST(@CurrentLogLSN AS nvarchar),'N/A');
    RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;
    ...
    BEGIN
      SET @DatabaseMessage = N'Allocated extent page count: ' + ISNULL(CAST(@CurrentAllocatedExtentPageCount AS nvarchar) + ' (' + CAST(@CurrentAllocatedExtentPageCount * 1. * 8 / 1024 AS nvarchar) + ' MB)','N/A');
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;

      SET @DatabaseMessage = N'Modified extent page count: ' + ISNULL(CAST(@CurrentModifiedExtentPageCount AS nvarchar) + ' (' + CAST(@CurrentModifiedExtentPageCount * 1. * 8 / 1024 AS nvarchar) + ' MB)','N/A');
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;
    END;
    ...
    BEGIN
      SET @DatabaseMessage = N'Last log backup: ' + ISNULL(CONVERT(nvarchar(19),NULLIF(@CurrentLastLogBackup,'1900-01-01'),120),'N/A');
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;

      SET @DatabaseMessage = N'Log size since last log backup (MB): ' + ISNULL(CAST(@CurrentLogSizeSinceLastLogBackup AS nvarchar),'N/A');
      RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;
    END;
    ...
    BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          SET @CurrentDatabaseContext = N'master';

          SET @CurrentCommandType = N'xp_create_subdir';

          SET @CurrentCommand = N'DECLARE @ReturnCode int EXECUTE @ReturnCode = dbo.xp_create_subdir N''' + REPLACE(@CurrentDirectoryPath,'''','''''') + N''' IF @ReturnCode <> 0 RAISERROR(''Error creating directory.'', 16, 1)';

          EXECUTE @CurrentCommandOutput = dba.CommandExecute @DatabaseContext = @CurrentDatabaseContext, @Command = @CurrentCommand, @CommandType = @CurrentCommandType, @Mode = 1, @DatabaseName = @CurrentDatabaseName, @LogToTable = @LogToTable, @Execute = @ExecutE
          SET @Error = @@ERROR;
      ...
      BEGIN
        ...
        BEGIN
          ...
          BEGIN
            ...
            SET @CurrentCommandType = N'xp_delete_file';

            SET @CurrentCommand = N'DECLARE @ReturnCode int EXECUTE @ReturnCode = dbo.xp_delete_file 0, N''' + REPLACE(@CurrentDirectoryPath,'''','''''') + N''', ''' + @CurrentFileExtension + N''', ''' + CONVERT(nvarchar(19),@CurrentCleanupDate,126) + N''' IF @ReturnCode <> 0 RAISERROR(''Error deleting files.'', 16, 1)';
          END;
          ...
          BEGIN
            ...
            SET @CurrentCommandType = N'xp_slssqlmaint';

            SET @CurrentCommand = N'DECLARE @ReturnCode int EXECUTE @ReturnCode = dbo.xp_slssqlmaint N''-MAINTDEL -DELFOLDER "' + REPLACE(@CurrentDirectoryPath,'''','''''') + N'" -DELEXTENSION "' + @CurrentFileExtension + N'" -DELUNIT "' + CAST(DATEDIFF(mi,@CurrentCleanupDate,SYSDATETIME()) + 1 AS nvarchar) + N'" -DELUNITTYPE "minutes" -DELUSEAGE'' IF @ReturnCode <> 0 RAISERROR(''Error deleting LiteSpeed backup files.'', 16, 1)';
          END;
          ...
          BEGIN
            ...
            SET @CurrentCommandType = N'sqbutility';

            SET @CurrentCommand = N'DECLARE @ReturnCode int EXECUTE @ReturnCode = dbo.sqbutility 1032, N''' + REPLACE(@CurrentDatabaseName,'''','''''') + N''', N''' + REPLACE(@CurrentDirectoryPath,'''','''''') + N''', ''' + CASE WHEN @CurrentBackupType = 'FULL' THEN 'D' WHEN @CurrentBackupType = 'DIFF' THEN 'I' WHEN @CurrentBackupType = 'LOG' THEN 'L' END + N''', ''' + CAST(DATEDIFF(hh,@CurrentCleanupDate,SYSDATETIME()) + 1 AS nvarchar) + N'h'', ' + ISNULL('''' + REPLACE(@EncryptionKey,'''','''''') + '''','NULL') + N' IF @ReturnCode <> 0 RAISERROR(''Error deleting SQLBackup backup files.'', 16, 1)';
          END;
          ...
          BEGIN
            ...
            SET @CurrentCommandType = N'xp_ss_delete';

            SET @CurrentCommand = N'DECLARE @ReturnCode int EXECUTE @ReturnCode = dbo.xp_ss_delete @filename = N''' + REPLACE(@CurrentDirectoryPath,'''','''''') + N'\*.' + @CurrentFileExtension + N''', @age = ''' + CAST(DATEDIFF(mi,@CurrentCleanupDate,SYSDATETIME()) + 1 AS nvarchar) + N'Minutes'' IF @ReturnCode <> 0 RAISERROR(''Error deleting SQLsafe backup files.'', 16, 1)';
          END;
      ...
      BEGIN
        ...
        BEGIN
          ...
          IF @ReadWriteFileGroups = 'Y' AND @CurrentDatabaseName <> 'master' SET @CurrentCommand += N' READ_WRITE_FILEGROUPS';

          SET @CurrentCommand += N' TO';

          SELECT @CurrentCommand += N' ' + [Type] + N' = N''' + REPLACE(FilePath,'''','''''') + N'''' + CASE WHEN ROW_NUMBER() OVER (ORDER BY FilePath ASC) <> @CurrentNumberOfFiles THEN ',' ELSE '' END
          FROM @CurrentFiles
          WHERE Mirror = 0
          ...
          BEGIN
            SET @CurrentCommand += N' MIRROR TO';
          ...
          SET @CurrentCommand += N' WITH STATS = 5, ';
          IF @CheckSum = 'Y' SET @CurrentCommand += N'CHECKSUM';
          IF @CheckSum = 'N' SET @CurrentCommand += N'NO_CHECKSUM';
          ...
          IF @CurrentBackupType = 'DIFF' SET @CurrentCommand += N', DIFFERENTIAL';

          IF EXISTS(SELECT * FROM @CurrentFiles WHERE Mirror = 1)
          BEGIN
            SET @CurrentCommand += N', FORMAT';
          END;

          IF @CopyOnly = 'Y' SET @CurrentCommand += N', COPY_ONLY';
          IF @NoRecovery = 'Y' AND @CurrentBackupType = 'LOG' SET @CurrentCommand += N', NORECOVERY';
          IF @Init = 'Y' SET @CurrentCommand += N', INIT';
          IF @Format = 'Y' SET @CurrentCommand += N', FORMAT';
          IF @BlockSize IS NOT NULL SET @CurrentCommand += N', BLOCKSIZE = ' + CAST(@BlockSize AS nvarchar);
          IF @BufferCount IS NOT NULL SET @CurrentCommand += N', BUFFERCOUNT = ' + CAST(@BufferCount AS nvarchar);
          IF @CurrentMaxTransferSize IS NOT NULL SET @CurrentCommand += N', MAXTRANSFERSIZE = ' + CAST(@CurrentMaxTransferSize AS nvarchar);
          IF @Description IS NOT NULL SET @CurrentCommand += N', DESCRIPTION = N''' + REPLACE(@Description,'''','''''') + N'''';
          IF @Encrypt = 'Y' SET @CurrentCommand += N', ENCRYPTION (ALGORITHM = ' + UPPER(@EncryptionAlgorithm) + N', ';
          IF @Encrypt = 'Y' AND @ServerCertificate IS NOT NULL SET @CurrentCommand += N'SERVER CERTIFICATE = ' + QUOTENAME(@ServerCertificate);
          IF @Encrypt = 'Y' AND @ServerAsymmetricKey IS NOT NULL SET @CurrentCommand += N'SERVER ASYMMETRIC KEY = ' + QUOTENAME(@ServerAsymmetricKey);
          IF @Encrypt = 'Y' SET @CurrentCommand += N')';
          IF @URL IS NOT NULL AND @Credential IS NOT NULL SET @CurrentCommand += N', CREDENTIAL = N''' + REPLACE(@Credential,'''','''''') + N'''';
        END;
        ...
        BEGIN
          ...
          SELECT @CurrentCommand += N', @filename = N''' + REPLACE(FilePath,'''','''''') + N''''
          FROM @CurrentFiles
          WHERE Mirror = 0
          ORDER BY FilePath ASC;
          ...
          IF EXISTS(SELECT * FROM @CurrentFiles WHERE Mirror = 1)
          BEGIN
            SELECT @CurrentCommand += N', @mirror = N''' + REPLACE(FilePath,'''','''''') + N''''
            FROM @CurrentFiles
            WHERE Mirror = 1
            ORDER BY FilePath ASC;
          END;

          SET @CurrentCommand += N', @with = ''';
          IF @CheckSum = 'Y' SET @CurrentCommand += N'CHECKSUM';
          IF @CheckSum = 'N' SET @CurrentCommand += N'NO_CHECKSUM';
          IF @CurrentBackupType = 'DIFF' SET @CurrentCommand += N', DIFFERENTIAL';
          IF @CopyOnly = 'Y' SET @CurrentCommand += N', COPY_ONLY';
          IF @NoRecovery = 'Y' AND @CurrentBackupType = 'LOG' SET @CurrentCommand += N', NORECOVERY';
          IF @BlockSize IS NOT NULL SET @CurrentCommand += N', BLOCKSIZE = ' + CAST(@BlockSize AS nvarchar);
          SET @CurrentCommand += N'''';
          IF @ReadWriteFileGroups = 'Y' AND @CurrentDatabaseName <> 'master' SET @CurrentCommand += N', @read_write_filegroups = 1';
          IF @CompressionLevel IS NOT NULL SET @CurrentCommand += N', @compressionlevel = ' + CAST(@CompressionLevel AS nvarchar);
          IF @AdaptiveCompression IS NOT NULL SET @CurrentCommand += N', @adaptivecompression = ''' + CASE WHEN @AdaptiveCompression = 'SIZE' THEN 'Size' WHEN @AdaptiveCompression = 'SPEED' THEN 'Speed' END + N'''';
          IF @BufferCount IS NOT NULL SET @CurrentCommand += N', @buffercount = ' + CAST(@BufferCount AS nvarchar);
          IF @CurrentMaxTransferSize IS NOT NULL SET @CurrentCommand += N', @maxtransfersize = ' + CAST(@CurrentMaxTransferSize AS nvarchar);
          IF @Threads IS NOT NULL SET @CurrentCommand += N', @threads = ' + CAST(@Threads AS nvarchar);
          IF @Init = 'Y' SET @CurrentCommand += N', @init = 1';
          IF @Format = 'Y' SET @CurrentCommand += N', @format = 1';
          IF @Throttle IS NOT NULL SET @CurrentCommand += N', @throttle = ' + CAST(@Throttle AS nvarchar);
          IF @Description IS NOT NULL SET @CurrentCommand += N', @desc = N''' + REPLACE(@Description,'''','''''') + N'''';
          IF @ObjectLevelRecoveryMap = 'Y' SET @CurrentCommand += N', @olrmap = 1';

          IF @EncryptionAlgorithm IS NOT NULL SET @CurrentCommand += N', @cryptlevel = ' + CASE
          ...
          IF @EncryptionKey IS NOT NULL SET @CurrentCommand += N', @encryptionkey = N''' + REPLACE(@EncryptionKey,'''','''''') + N'''';
          SET @CurrentCommand += N' IF @ReturnCode <> 0 RAISERROR(''Error performing LiteSpeed backup.'', 16, 1)';
        END;
        ...
        BEGIN
          ...
          SET @CurrentCommandType = N'sqlbackup';
          ...
          SELECT @CurrentCommand += N' DISK = N''' + REPLACE(FilePath,'''','''''') + N'''' + CASE WHEN ROW_NUMBER() OVER (ORDER BY FilePath ASC) <> @CurrentNumberOfFiles THEN ',' ELSE '' END
          FROM @CurrentFiles
          WHERE Mirror = 0
          ORDER BY FilePath ASC;
          ...
          IF EXISTS(SELECT * FROM @CurrentFiles WHERE Mirror = 1)
          BEGIN
            SET @CurrentCommand += N' MIRRORFILE' + N' = N''' + REPLACE((SELECT FilePath FROM @CurrentFiles WHERE Mirror = 1),'''','''''') + N''', ';
          END;
          ...
          IF @Init = 'Y' SET @CurrentCommand += N', INIT';
          IF @Format = 'Y' SET @CurrentCommand += N', FORMAT';
          IF @CompressionLevel IS NOT NULL SET @CurrentCommand += N', COMPRESSION = ' + CAST(@CompressionLevel AS nvarchar);
          IF @Threads IS NOT NULL SET @CurrentCommand += N', THREADCOUNT = ' + CAST(@Threads AS nvarchar);
          IF @CurrentMaxTransferSize IS NOT NULL SET @CurrentCommand += N', MAXTRANSFERSIZE = ' + CAST(@CurrentMaxTransferSize AS nvarchar);
          IF @Description IS NOT NULL SET @CurrentCommand += N', DESCRIPTION = N''' + REPLACE(@Description,'''','''''') + N'''';

          IF @EncryptionAlgorithm IS NOT NULL SET @CurrentCommand += N', KEYSIZE = ' + CASE
          WHEN @EncryptionAlgorithm = 'AES_128' THEN '128'
          WHEN @EncryptionAlgorithm = 'AES_256' THEN '256'
          END;

          IF @EncryptionKey IS NOT NULL SET @CurrentCommand += N', PASSWORD = N''' + REPLACE(@EncryptionKey,'''','''''') + N'''';
          SET @CurrentCommand = N'DECLARE @ReturnCode int EXECUTE @ReturnCode = dbo.sqlbackup N''-SQL "' + REPLACE(@CurrentCommand,'''','''''') + N'"''' + N' IF @ReturnCode <> 0 RAISERROR(''Error performing SQLBackup backup.'', 16, 1)';
        END;
        ...
        BEGIN
          ...
          SET @CurrentCommandType = N'xp_ss_backup';

          SET @CurrentCommand = N'DECLARE @ReturnCode int EXECUTE @ReturnCode = dbo.xp_ss_backup @database = N''' + REPLACE(@CurrentDatabaseName,'''','''''') + N'''';

          SELECT @CurrentCommand += N', ' + CASE WHEN ROW_NUMBER() OVER (ORDER BY FilePath ASC) = 1 THEN '@filename' ELSE '@backupfile' END + N' = N''' + REPLACE(FilePath,'''','''''') + N''''
          FROM @CurrentFiles
          WHERE Mirror = 0
          ORDER BY FilePath ASC;

          SELECT @CurrentCommand += N', @mirrorfile = N''' + REPLACE(FilePath,'''','''''') + N''''
          FROM @CurrentFiles
          WHERE Mirror = 1
          ORDER BY FilePath ASC;

          SET @CurrentCommand += N', @backuptype = ' + CASE WHEN @CurrentBackupType = 'FULL' THEN '''Full''' WHEN @CurrentBackupType = 'DIFF' THEN '''Differential''' WHEN @CurrentBackupType = 'LOG' THEN '''Log''' END;
          IF @ReadWriteFileGroups = 'Y' AND @CurrentDatabaseName <> 'master' SET @CurrentCommand += N', @readwritefilegroups = 1';
          SET @CurrentCommand += N', @checksum = ' + CASE WHEN @CheckSum = 'Y' THEN '1' WHEN @CheckSum = 'N' THEN '0' END;
          SET @CurrentCommand += N', @copyonly = ' + CASE WHEN @CopyOnly = 'Y' THEN '1' WHEN @CopyOnly = 'N' THEN '0' END;
          IF @CompressionLevel IS NOT NULL SET @CurrentCommand += N', @compressionlevel = ' + CAST(@CompressionLevel AS nvarchar);
          IF @Threads IS NOT NULL SET @CurrentCommand += N', @threads = ' + CAST(@Threads AS nvarchar);
          IF @Init = 'Y' SET @CurrentCommand += N', @overwrite = 1';
          IF @Description IS NOT NULL SET @CurrentCommand += N', @desc = N''' + REPLACE(@Description,'''','''''') + N'''';

          IF @EncryptionAlgorithm IS NOT NULL SET @CurrentCommand += N', @encryptiontype = N''' + CASE
          WHEN @EncryptionAlgorithm = 'AES_128' THEN 'AES128'
          WHEN @EncryptionAlgorithm = 'AES_256' THEN 'AES256'
          END + N'''';

          IF @EncryptionKey IS NOT NULL SET @CurrentCommand += N', @encryptedbackuppassword = N''' + REPLACE(@EncryptionKey,'''','''''') + N'''';
          SET @CurrentCommand += N' IF @ReturnCode <> 0 RAISERROR(''Error performing SQLsafe backup.'', 16, 1)';
        END;
        ...
        BEGIN
          ...
          SET @CurrentCommandType = N'emc_run_backup';

          SET @CurrentCommand = N'DECLARE @ReturnCode int EXECUTE @ReturnCode = dbo.emc_run_backup ''';

          SET @CurrentCommand += N' -c ' + CASE WHEN @CurrentAvailabilityGroup IS NOT NULL THEN @Cluster ELSE CAST(SERVERPROPERTY('MachineName') AS nvarchar) END;

          SET @CurrentCommand += N' -l ' + CASE
          WHEN @CurrentBackupType = 'FULL' THEN 'full'
          WHEN @CurrentBackupType = 'DIFF' THEN 'diff'
          WHEN @CurrentBackupType = 'LOG' THEN 'incr'
          END;

          IF @NoRecovery = 'Y' SET @CurrentCommand += N' -H';

          IF @CleanupTime IS NOT NULL SET @CurrentCommand += N' -y +' + CAST(@CleanupTime/24 + CASE WHEN @CleanupTime%24 > 0 THEN 1 ELSE 0 END AS nvarchar) + N'd';

          IF @CheckSum = 'Y' SET @CurrentCommand += N' -k';

          SET @CurrentCommand += N' -S ' + CAST(@CurrentNumberOfFiles AS nvarchar);

          IF @Description IS NOT NULL SET @CurrentCommand += N' -b "' + REPLACE(@Description,'''','''''') + N'"';

          IF @BufferCount IS NOT NULL SET @CurrentCommand += N' -O "BUFFERCOUNT=' + CAST(@BufferCount AS nvarchar) + N'"';

          IF @ReadWriteFileGroups = 'Y' AND @CurrentDatabaseName <> 'master' SET @CurrentCommand += N' -O "READ_WRITE_FILEGROUPS"';

          IF @DataDomainBoostHost IS NOT NULL SET @CurrentCommand += N' -a "NSR_DFA_SI_DD_HOST=' + REPLACE(@DataDomainBoostHost,'''','''''') + N'"';
          IF @DataDomainBoostUser IS NOT NULL SET @CurrentCommand += N' -a "NSR_DFA_SI_DD_USER=' + REPLACE(@DataDomainBoostUser,'''','''''') + N'"';
          IF @DataDomainBoostDevicePath IS NOT NULL SET @CurrentCommand += N' -a "NSR_DFA_SI_DEVICE_PATH=' + REPLACE(@DataDomainBoostDevicePath,'''','''''') + N'"';
          IF @DataDomainBoostLockboxPath IS NOT NULL SET @CurrentCommand += N' -a "NSR_DFA_SI_DD_LOCKBOX_PATH=' + REPLACE(@DataDomainBoostLockboxPath,'''','''''') + N'"';
          SET @CurrentCommand += N' -a "NSR_SKIP_NON_BACKUPABLE_STATE_DB=TRUE"';
          IF @CopyOnly = 'Y' SET @CurrentCommand += N' -a "NSR_COPY_ONLY=TRUE"';

          IF SERVERPROPERTY('InstanceName') IS NULL SET @CurrentCommand += N' "MSSQL' + N':' + REPLACE(REPLACE(@CurrentDatabaseName,'''',''''''),'.','\.') + N'"';
          IF SERVERPROPERTY('InstanceName') IS NOT NULL SET @CurrentCommand += N' "MSSQL$' + CAST(SERVERPROPERTY('InstanceName') AS nvarchar) + N':' + REPLACE(REPLACE(@CurrentDatabaseName,'''',''''''),'.','\.') + N'"';

          SET @CurrentCommand += N'''';

          SET @CurrentCommand += N' IF @ReturnCode <> 0 RAISERROR(''Error performing Data Domain Boost backup.'', 16, 1)';
        END;
      ...
      BEGIN
        ...
        BEGIN
          ...
          BEGIN
            ...
            SET @CurrentCommandType = N'RESTORE_VERIFYONLY';

            SET @CurrentCommand = N'RESTORE VERIFYONLY FROM';
          ...
          BEGIN
            ...
            SET @CurrentCommandType = N'xp_restore_verifyonly';

            SET @CurrentCommand = N'DECLARE @ReturnCode int EXECUTE @ReturnCode = dbo.xp_restore_verifyonly';

            SELECT @CurrentCommand += N' @filename = N''' + REPLACE(FilePath,'''','''''') + N'''' + CASE WHEN ROW_NUMBER() OVER (ORDER BY FilePath ASC) <> @CurrentNumberOfFiles THEN ',' ELSE '' END
            FROM @CurrentFiles
            WHERE Mirror = @CurrentIsMirror
            ORDER BY FilePath ASC;
            ...
            SET @CurrentCommand += N' IF @ReturnCode <> 0 RAISERROR(''Error verifying LiteSpeed backup.'', 16, 1)';
          END;
          ...
          BEGIN
            ...
            SET @CurrentCommand = N'DECLARE @ReturnCode int EXECUTE @ReturnCode = dbo.sqlbackup N''-SQL "' + REPLACE(@CurrentCommand,'''','''''') + N'"''' + N' IF @ReturnCode <> 0 RAISERROR(''Error verifying SQLBackup backup.'', 16, 1)';
          END;
          ...
          BEGIN
            ...
            SET @CurrentCommandType = N'xp_ss_verify';

            SET @CurrentCommand = N'DECLARE @ReturnCode int EXECUTE @ReturnCode = dbo.xp_ss_verify @database = N''' + REPLACE(@CurrentDatabaseName,'''','''''') + N'''';
            ...
            SET @CurrentCommand += N' IF @ReturnCode <> 0 RAISERROR(''Error verifying SQLsafe backup.'', 16, 1)';
          END;
    ...
    BEGIN
      SET @ErrorMessage = N'The database ' + QUOTENAME(@CurrentDatabaseName) + N' is in a SUSPECT state.';
      RAISERROR('%s',16,1,@ErrorMessage) WITH NOWAIT;
      SET @Error = @@ERROR;
      RAISERROR(@EmptyLine,10,1) WITH NOWAIT;
    END;
  ...
  Logging:
  SET @EndMessage = N'Date and time: ' + CONVERT(nvarchar,SYSDATETIME(),120);
