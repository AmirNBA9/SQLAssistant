...
FROM dbo.sysjobs AS a
    LEFT JOIN dbo.sysjobhistory AS b
...
/*Jobs_MinMaxAverageTime with steps, time format*/
WITH t
AS (SELECT a.job_id,
           b.step_name,
           b.step_id,
           ((b.run_duration / 10000 * 3600 + (b.run_duration / 100) % 100 * 60 + b.run_duration % 100 + 31) / 60) AS run_duration,
           CAST(DATEADD(
                           MILLISECOND,
                           ((b.run_duration / 10000 * 3600 + (b.run_duration / 100) % 100 * 60 + b.run_duration % 100)),
                           '00:00:00'
                       ) AS TIME(3)) AS run_duration_Format
    FROM dbo.sysjobs AS a
        LEFT JOIN dbo.sysjobhistory AS b
            ON b.job_id = a.job_id)
SELECT a.name AS 'JobName',
